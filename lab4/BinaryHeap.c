...
void printHeap(int *heap, int size, int node=0, int d=0) {
    ...
    if(heap != NULL){
        if (node<size) {
            ...
            for(int i = d; i > 0; i--){
                cout << "*" ;                  // indicating depth
            }
            cout << *(heap+node) << "\n";
            printHeap(heap,size,2*node+1,d+1); // go left
            printHeap(heap,size,2*node+2,d+1); // go right
        }
    }
    */
    // preorder traversal
...
void remove(int* heap, int key, int & size, int node) {
    ...
    remove(heap,key,size,(node<<1)+1);
    remove(heap,key,size,(node<<1)+2);
    
    // doesn't need heapify 
}
...
void remove(int *heap, int key, int &size) {
    ...
    // int & size can get access to actual size and change value of size
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
    ...
    int* merged_p = new int[sizes]; // new heap
...
int input4[] = {1,1,2,1,3,1};

int main() {
    int size10 = sizeof(input4)/sizeof(int);
    int *heap10 = new int[size10];
    for (int i = 0; i < size10; ++i) {
        heap10[i] = input4[i];
    }
    
    heapify(heap10, size10);
    cout << "heap1: ";
    printList(heap10, size10);
    printHeap(heap10, size10);
    
    remove(heap10, 1, size10);
    printHeap(heap10,size10);
